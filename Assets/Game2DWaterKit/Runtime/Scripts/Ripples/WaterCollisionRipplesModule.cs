        ...
        private void CreateWaterEnterExitRipples(Collider2D objectCollider, bool isObjectEnteringWater)
        {
            ...
            for (int surfaceVertexIndex = startIndex; surfaceVertexIndex <= endIndex; surfaceVertexIndex++)
            {
                ...
                if (hitCount > 0)
                {
                    ...
                    float velRespectNormal = Vector2.Dot(hit.rigidbody.GetPointVelocity(hit.point).normalized, hit.normal) * hit.rigidbody.GetPointVelocity(hit.point).magnitude;
                    float velocity = velRespectNormal * _velocityMultiplier;
        ...
        private void CreateWaterMoveRipples()
        {
            ...
            if (inWaterColliders.Count > 0)
            {
                ...
                foreach (var collider in inWaterColliders)
                {
                    ...
                    float xVelocity = Vector2.Dot(collider.attachedRigidbody.velocity.normalized, Vector2.Perpendicular(_mainModule.UpDirection)) * collider.attachedRigidbody.velocity.magnitude;
