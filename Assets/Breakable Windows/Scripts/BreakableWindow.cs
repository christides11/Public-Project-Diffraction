        ...
        if (isBroken == false)
        {
            if (allreadyCalculated == true)
            {
                ...
                if (addTorques)
                {
                    ...
                    {
                        var splinter = splinters[i].GetComponent<Rigidbody>();
                        splinter.AddTorque(new Vector3(Random.value > 0.5f ? Random.value * 50 : -Random.value * 50, Random.value > 0.5f ? Random.value * 50 : -Random.value * 50, Random.value > 0.5f ? Random.value * 50 : -Random.value * 50));
                        splinter.AddForce(new Vector3((Random.value - 0.5f) * 10, (Random.value - 0.5f) * 10, (Random.value - 0.5f) * 10), ForceMode.Impulse);
                    }
                }
            }
            else
